...
namespace iter {
  ...
  class Zipped<Container, RestContainers...> {
    using ZipIterDeref = std::tuple<iterator_deref<Container>,
    ...
    class Iterator
     ...
     public:
      ...
      auto operator*() -> decltype(
          std::tuple_cat(std::tuple<iterator_deref<Container>>{*this->iter},
              *this->rest_iter)) {
        return std::tuple_cat(
            std::tuple<iterator_deref<Container>>{*this->iter},
            *this->rest_iter);
      }
