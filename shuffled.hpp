 ...
 private:
  ...
  ShuffledView(Container&& in_container, int in_seed)
      : container(std::forward<Container>(in_container)),
        ...
        size_approx(get_approx(size)),
        seed(in_seed) {
    if (size > 0) {
      std::uint64_t mask = -1;
      mask = (mask >> (64 - size_approx));
      this->seed = seed & mask;
      this->seed = shift(this->seed, size_approx);
  ...
  static constexpr std::array<std::uint16_t, 64> PRIME_POLY = {{1, 3, 3, 3, 5,
      3, 3, 29, 17, 9, 5, 83, 27, 43, 3, 45, 9, 129, 39, 9, 5, 3, 33, 27, 9, 71,
      39, 9, 5, 83, 9, 197, 8193, 231, 5, 119, 83, 99, 17, 57, 9, 153, 89, 101,
      27, 449, 33, 183, 113, 29, 75, 9, 71, 125, 71, 149, 129, 99, 123, 3, 39,
      105, 3, 27}};  // prime poly for full-cycle shift registers
  // power of 2 approximation (val < pow(2, get_approx(val)+1))
  static constexpr inline std::uint16_t get_approx(std::uint64_t val) {
    if (val == 0) {
      return 0;
    }
  ...
  static constexpr inline std::uint64_t shift(
  ...
  class Iterator : public std::iterator<std::input_iterator_tag, IterDeref> {
   ...
   public:
    ...
    Iterator& operator++() {
      ...
      do {
        state = shift(state, owner->size_approx);
...
template <typename Container, typename Distance>
constexpr std::array<std::uint16_t, 64>
    iter::impl::ShuffledView<Container, Distance>::PRIME_POLY;
