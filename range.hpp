...
namespace iter {
  ...
  class RangeIterData<T, false> {
   ...
   public:
    ...
    constexpr RangeIterData(T in_value, T in_step) noexcept : value_{in_value},
                                                              step_{in_step} {}
  ...
  class RangeIterData<T, true> {
   ...
   public:
    constexpr RangeIterData() noexcept = default;
    constexpr RangeIterData(T in_start, T in_step) noexcept : start_{in_start},
                                                              value_{in_start},
                                                              step_{in_step} {}
  ...
  template <typename T>
  constexpr Range<T> range(T stop) noexcept {
    return {stop};
  }

  template <typename T>
  constexpr Range<T> range(T start, T stop) noexcept {
    return {start, stop};
  }

  template <typename T>
  constexpr Range<T> range(T start, T stop, T step) noexcept {
